...
def checkin_fetchall():
    if 'feeling' in request.args and 'squad' in request.args:
        feeling = request.args['feeling']
        squad = request.args['squad']
        sql = ("SELECT * FROM checkins WHERE Feeling = {}".format(feeling) + " AND Squad = {}".format(
            squad) + " ORDER BY Date DESC")
        myCursor.execute(sql)
        myResults = myCursor.fetchall()
    elif 'feeling' in request.args:
        feeling = request.args['feeling']
        sql = ("SELECT * FROM checkins WHERE Feeling = {}".format(feeling) + " ORDER BY Date DESC")
        myCursor.execute(sql)
        myResults = myCursor.fetchall()
    elif 'squad' in request.args:
        squad = request.args['squad']
        sql = ("SELECT * FROM checkins WHERE Squad = {}".format(squad) + " ORDER BY Date DESC")
        myCursor.execute(sql)
        myResults = myCursor.fetchall()
    else:
        myCursor.execute('SELECT * FROM checkins ORDER BY Date DESC')
        myResults = myCursor.fetchall()
...
def user_fetch(user_id):
    if 'feeling' in request.args and 'squad' in request.args:
        feeling = request.args['feeling']
        squad = request.args['squad']
        sql = ("SELECT * FROM checkins WHERE User_ID = {}".format(user_id) + " AND Feeling = {}".format(
            feeling) + " AND Squad = {}".format(squad) + " ORDER BY Date DESC")
        myCursor.execute(sql)
        myResults = myCursor.fetchall()
    elif 'feeling' in request.args:
        feeling = request.args['feeling']
        sql = ("SELECT * FROM checkins WHERE User_ID = {}".format(user_id) + " AND Feeling = {}".format(
            feeling) + " ORDER BY Date DESC")
        myCursor.execute(sql)
        myResults = myCursor.fetchall()
    elif 'squad' in request.args:
        squad = request.args['squad']
        sql = ("SELECT * FROM checkins WHERE User_ID = {}".format(user_id) + " AND Squad = {}".format(
            squad) + " ORDER BY Date DESC")
        myCursor.execute(sql)
        myResults = myCursor.fetchall()
    else:
...
def checkins_post():
    data = request.json
    myCursor.execute(
        "INSERT INTO checkins (Start_time, Completion_time, User_ID, Date, Squad, Feeling, Why_feeling, Did, Learned, Todo, Question) VALUES ('{}', '{}', '{}', '{}', '{}', '{}','{}', '{}','{}','{}', '{}')".format(
            data['start_time'], data['getTime'], data['User_ID'], data['date'], data['team'], data['feeling'],
            data['why_feeling'], data['did'], data['learned'], data['todo'], data['question']))
    connection.commit()
